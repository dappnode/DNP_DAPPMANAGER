...
RUN apk add --no-cache docker curl docker-compose bind-dev xz libltdl miniupnpc zip unzip
...
COPY --from=build-binaries /usr/bin/nsupdate /usr/bin/docker \
  ...
  /usr/bin/curl /usr/bin/upnpc /usr/bin/zip /usr/bin/unzip /usr/local/bin/

# TODO: Try to install these in the build stage
RUN apk add --no-cache dbus bind-tools avahi-tools
