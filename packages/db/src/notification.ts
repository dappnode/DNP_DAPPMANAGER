...
import { PackageNotificationDb, PackageNotification } from "@dappnode/types";
...
export const notification = dbCache.indexedByKey<PackageNotificationDb, string>({
  ...
  validate: (id, notification) => typeof id === "string" && typeof notification === "object"
});
...
export const notificationLastEmitVersion = dbCache.indexedByKey<string, string>({
  ...
  validate: (dnpName, lastEmittedVersion) => Boolean(dnpName) && typeof lastEmittedVersion === "string"
});
