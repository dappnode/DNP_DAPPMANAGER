...
import { parsePortMappings, stringifyPortMappings, mergePortMappings, mergePortArrays } from "../../src/index.js";
...
describe("portMappings: parse, stringify and merge", () => {
  it("should parse and stringify port mappings", () => {
    ...
    expect(parsePortMappings(portArray)).to.deep.equal(portMappings, "Wrong parse");

    expect(stringifyPortMappings(portMappings)).to.deep.equal(portArray, "Wrong stringify");
  });
  ...
  it("should parse, merge and stringify two port arrays", () => {
    ...
    expect(mergedPortMappings).to.deep.equal(["30656:30303/udp", "8080:8080", "4001:4001"]);
  });
});
