...
export async function validateComposeSchema(composePaths: string[]): Promise<void> {
  if (composePaths.length < 1) throw Error(`No compose files provided`);

  composePaths.forEach((composePath) => {
    if (!fs.existsSync(composePath)) throw Error(`Compose file ${composePath} not found`);
  });

  return new Promise((resolve, reject) => {
    exec(`docker compose -f ${composePaths.join(" -f ")} config`, (error, _stdout, stderr) => {
      if (error) {
