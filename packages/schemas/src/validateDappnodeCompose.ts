 ...
 */
function validateComposeService(compose: Compose, isCore: boolean, serviceName: string, dnpName: string): void {
  ...
  if (pid && !pid.startsWith("service:")) err(`service ${serviceName} has PID feature differnet than service:*`);

  // Check only core packages or exporter cand be privileged
  if (dnpName !== "dappnode-exporter.dnp.dappnode.eth" && !isCore && privileged && privileged === true)
  ...
  if (volumes && !dockerParams.DOCKER_WHITELIST_BIND_VOLUMES.includes(dnpName)) {
 ...
 */
function validateComposeServiceNetworks(compose: Compose, isCore: boolean, serviceName: string): void {
  ...
  if (Array.isArray(serviceNetworks)) {
    ...
    for (const serviceNetworkObjectName of Object.keys(serviceNetworks)) {
      ...
      if (!isCore && aliases && dockerParams.DOCKER_CORE_ALIASES.some((coreAlias) => aliases.includes(coreAlias))) {
        err(
 ...
 */
function validateComposeServiceVolumes(compose: Compose, serviceName: string, volume: string): void {
