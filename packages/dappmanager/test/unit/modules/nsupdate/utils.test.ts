...
describe("modules > nsupdate", () => {
  ...
  describe("getDotDappnodeDomain", () => {
    ...
    for (const [name, domain] of Object.entries(cases)) {
      it(`Should get the domain of ${name}`, () => {
        expect(
          getDotDappnodeDomain({ dnpName: name, serviceName: "" })
        ).to.equal(domain);
      });
    }
  });
...
update delete dms.dnp.dappnode.eth.dms.dappnode A
update add dms.dnp.dappnode.eth.dms.dappnode 60 A 172.33.0.3`
      });
    });
  });
});
