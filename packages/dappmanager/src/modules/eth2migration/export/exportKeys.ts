...
--wallet-password-file=${eth2migrationParams.keys.walletPasswordFile} --${network} --accept-terms-of-use`
    ...
    await shell(
      `docker exec ${containerName} cp ${eth2migrationParams.keys.walletPasswordFile} ${eth2migrationParams.backup.backupDir}`
    ).catch(e => {
...
--${network} --accept-terms-of-use`
    ...
    await shell(
      `docker exec ${containerName} sh -c 'apt update; apt install -y unzip; unzip -p ${eth2migrationParams.backup.backupKeysFile} -d ${eth2migrationParams.backup.backupDir}'`
    ).catch(e => {
