...
import { stopAvahiDaemon, startAvahiDaemon, getAvahiDaemonStatus } from "@dappnode/hostscriptsservices";
 ...
 */
export async function localProxyingEnableDisable(enable: boolean): Promise<void> {
...
export async function localProxyingStatusGet(): Promise<LocalProxyingStatus> {
  ...
  const LOCAL_PROXYING =
    userSettings.environment?.[params.HTTPS_PORTAL_MAIN_SERVICE]?.[params.HTTPS_PORTAL_LOCAL_PROXYING_ENVNAME];
...
// Utils
function parseLocalProxyingStatus(avahiStatus: AvahiDaemonStatus, localProxyingEnabled: boolean): LocalProxyingStatus {
