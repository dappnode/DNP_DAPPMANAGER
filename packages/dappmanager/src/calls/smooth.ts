...
export async function getShouldShowSmooth(): Promise<boolean> {
  ...
  if (await listPackageNoThrow({ dnpName: "web3signer.dnp.dappnode.eth" })) return true;
  else return false;
}
 ...
 */
export async function setShouldShownSmooth({ isShown }: { isShown: boolean }): Promise<void> {
