...
export function validateBackupArray(backup: PackageBackup[]): void {
  ...
  for (const { name, path: _path } of backup) {
    if (pathsObj[name]) throw Error(`Backup name ${name} is duplicated. They must be unique`);
    if (!path.isAbsolute(_path)) throw Error(`Backup paths must be absolute, path: ${_path}`);
    if (path.parse(name).dir) throw Error(`Backup names cannot be a path, name: ${name}`);
    pathsObj[name] = true;
  }
}
