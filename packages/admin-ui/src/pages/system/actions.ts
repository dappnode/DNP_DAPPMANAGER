...
export const changeEthClientTarget = (nextTarget: Eth2ClientTarget, newEthRemoteRpc: string): AppThunk => async (
  _,
  getState
) => {
  ...
  if (isEqual(nextTarget, prevTarget) && prevEthRemoteRpc === newEthRemoteRpc) return;
  ...
  if (prevTarget && prevTarget !== "remote") {
    if (nextTarget === "remote" || nextTarget.execClient !== prevTarget.execClient) {
    ...
    // If the previous target is package, ask the user if deleteVolumes
    if (nextTarget === "remote" || nextTarget.consClient !== prevTarget.consClient) {
      await new Promise<void>((resolve) =>
        confirm({
