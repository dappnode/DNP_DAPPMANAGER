...
export default function VolumesGrid() {
  ...
  return (
    <Card className={"list-grid volumes " + joinCssClass({ showRemove, showMountpoint })}>
      <header>Name</header>
        ...
        return (
          <React.Fragment key={name}>
            <div className="name">
              <span className="text">
                {owner ? (
                  <NavLink className="owner" to={`${packagesRootPath}/${owner}`}>
                    {ownerPretty}
                  </NavLink>
                ) : ownerPretty ? (
            ...
            <div className="size">
              {!size && fileSystem ? (
                <OverlayTrigger
                  ...
                  overlay={(props: any) => (
                    <Tooltip {...props}>
                      Can't get the exact volume size, use the mountpoint total size as an approximate upper reference
                    </Tooltip>
                  )}
                >
            ...
            {showMountpoint &&
              (fileSystem ? <MountpointDataView fileSystem={fileSystem}></MountpointDataView> : "Docker volume")}
            <div className="created-at">{parseStaticDate(createdAt, true)}</div>
            {showRemove && <MdDelete className={isDeletable ? "" : "disabled"} onClick={onDelete} />}
            <hr />
          </React.Fragment>
        );
      })}
