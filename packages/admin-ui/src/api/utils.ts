 ...
 */
export function continueIfCalleDisconnected(fn: () => Promise<void>, dnpName: string): () => Promise<void> {
  return async function (...args) {
    try {
      await fn(...args);
    } catch (e) {
      if (isCallDisconnectedError(e) && (!dnpName || dnpName === dappmanagerDnpName || dnpName === coreDnpName)) return;
      else throw e;
    }
  };
}
