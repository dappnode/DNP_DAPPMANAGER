...
export default function fileToDataUri(file: File): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      ...
      if (!e || !e.target || !e.target.result) return reject(Error("e.target fileContent is not defined"));
      const fileContent = e.target.result;
      resolve(fileContent instanceof ArrayBuffer ? fileContent.toString() : fileContent);
    };
  });
}
