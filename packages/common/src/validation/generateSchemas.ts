...
if (!tsConfigPath || !baseDir) throw Error("Requires 2 positional arguments: <tsConfigPath> <baseDir>");

const typesToSchema = ["RoutesArguments", "RoutesReturn", "SubscriptionsArguments"];
...
const getJsonPath = (typeName: string) => path.join(baseDir, `${typeName}.schema.json`);
...
for (const typeName of typesToSchema) {
  ...
  for (const route in schema.properties) {
    ...
    if (typeof prop !== "boolean" && prop.type === "array" && !prop.items) delete schema.properties[route];
  }

  fs.writeFileSync(getJsonPath(typeName), JSON.stringify(schema, null, 2));
  fs.writeFileSync(getTsPath(typeName), `export default ${JSON.stringify(schema)};`);
}
