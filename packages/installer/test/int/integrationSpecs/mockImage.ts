...
import { validatePath, getImageTag } from "@dappnode/utils";

const dockerContextPath = path.resolve(path.dirname(fileURLToPath(import.meta.url)));
...
export async function saveNewImageToDisk(
  { dnpName, version, serviceNames }: { dnpName: string; version: string; serviceNames: string[] },
  dirToSaveTo: string = testDir
): Promise<string> {
  const imageTag = await buildMockImage();

  if (!serviceNames) serviceNames = [dnpName];
  const newImageTags = serviceNames.map((serviceName) => getImageTag({ dnpName, serviceName, version }));
  for (const newImageTag of newImageTags) await shell(["docker", "tag", imageTag, newImageTag]);

  const newImagePath = path.resolve(dirToSaveTo, `${dnpName}_${version}.tar.xz`);
