...
export async function cleanInstallationArtifacts(partialContainerName: string): Promise<void> {
  await cleanRepos();
  const containersToKill = await shell(`docker ps -f name=${partialContainerName} -aq`);
