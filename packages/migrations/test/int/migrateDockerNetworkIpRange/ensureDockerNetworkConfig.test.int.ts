...
describe("Ensure docker network config migration => ensureDockerNetworkConfig", () => {
  ...
  before(async () => {
    ...
    await Promise.all(containerNames.map(async (cn) => await dockerNetworkConnect(dockerNetworkName, cn)));
  });
  ...
  it("should ensure the docker network config", async () => {
    ...
    const recreatedNetwork: Dockerode.NetworkInspectInfo = await docker.getNetwork(dockerNetworkName).inspect();
    ...
    expect(recreatedNetworkConfig[0].Subnet).to.deep.equal(newDockerNetworkSubnet);
    // validate all containers are connected
    const containers = ((await docker.getNetwork(dockerNetworkName).inspect()) as Dockerode.NetworkInspectInfo)
      .Containers;
    if (!containers) throw Error("containers not exist");
    const containerNamesConnected = Object.values(containers).map((c) => c.Name);
