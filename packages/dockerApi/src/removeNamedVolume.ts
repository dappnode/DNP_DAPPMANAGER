...
export async function removeNamedVolume(volName: string): Promise<void> {
  ...
  if (volInfo.Options && volInfo.Options.device && volInfo.Driver === "local" && volInfo.Options.o === "bind") {
    ...
    if (!devicePath.includes(params.MOUNTPOINT_DEVICE_PREFIX))
      throw Error(`devicePath must contain the volume tag '${params.MOUNTPOINT_DEVICE_PREFIX}': ${devicePath}`);
    if (devicePath.length < 10) throw Error(`devicePath is too short: ${devicePath}`);
