...
function orderDependencies(packageList) {
  ...
  // Validate
  if (!Array.isArray(packageList)) throw Error('packageList is not an array, packageList: '+JSON.stringify(packageList))
  packageList.map((pkg) => {
    if (!pkg.hasOwnProperty('name')) throw Error('packageList pkg does not contain a name key, pkg: '+JSON.stringify(pkg))
    if (!pkg.hasOwnProperty('dep'))  throw Error('packageList pkg does not contain a dep  key, pkg: '+JSON.stringify(pkg))
    if (Array.isArray(pkg.dep)) throw Error('packageList pkg.dep should not be an array, pkg.dep: '+JSON.stringify(pkg.dep))
  })
