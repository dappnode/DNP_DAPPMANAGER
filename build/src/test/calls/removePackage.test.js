...
describe('Call function: removePackage', function() {
  ...
  const portsToClose = [{number: 32769, type: 'UDP'}, {number: 32769, type: 'TCP'}];
  ...
  it('should have called docker-compose down', async () => {
    ...
    expect(docker.compose.down.getCall(0).args).to.deep.equal([dockerComposePath, {volumes: false}], `should call docker.compose.down for the package ${id}`);
  });
