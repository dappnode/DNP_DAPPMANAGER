...
describe("ipfsTasksFactory", () => {
  ...
  describe("isFileHashValid", () => {
    ...
    before("Create files for the test", async () => {
      ...
      await promisify(fs.writeFile)(PATH2, "Something", "utf8").catch(
        logs.error
      );
    });
  ...
  describe("downloadHandler", () => {
    ...
    before("Create files for the test", async () => {
      await promisify(fs.writeFile)(PATH_SOURCE, FILE_CONTENT, "utf8").catch(
        logs.error
      );
    });
    ...
    it("Should download a fake hello-world file", () => {
      const logChunks = () => {};
      return ipfsTasks.downloadHandler(HASH, PATH, logChunks).then(
        res => {
    ...
    it("Should detect a downloaded file is corrupt", () => {
      ...
      const logChunks = () => {};
      return ipfsTasks.downloadHandler(HASH_FAKE, PATH, logChunks).then(
        res => {
          expect(res).to.be.undefined;
        },
        err => {
  ...
  describe("cat & download", () => {
    ...
    it("Should download a fake hello-world file", () => {
      const HASH = "QmeV1kwh3333bsnT6YRfdCRrSgUPngKmAhhTa4RrqYPbKT";
      return ipfsTasks.download(HASH, PATH).then(
        res => {
          expect(res).to.be.undefined;
    ...
    it("Should cat a fake hello-world file", () => {
      const HASH = "QmeV1kwh3333bsnT6YRfdCRrSgUPngKmAhhTa4RrqYPbKT";
      return ipfsTasks.cat(HASH).then(
        res => {
