...
describe('semver-logic', () => {
    it('valid versions', async () => {
        ...
        Object.keys(versions).forEach((version) => {
            expect(Boolean(semver.valid(version))).to.equal(versions[version], `Version ${version} should ${versions[version] ? 'be' : 'NOT be'} valid`);
        });
    });
    ...
    it('satisfy 1.2.3', async () => {
        ...
        for (const range of Object.keys(ranges)) {
            for (const version of Object.keys(versions)) {
                console.log(`[version]  ${version}  [range]  ${range}  => ${Boolean(semver.satisfies(version, range))}`);
            }
        }
        // Object.keys(versions).forEach((version) => {
    ...
    it('gt lt', async () => {
        ...
        for (const version2 of Object.keys(versions2)) {
            for (const version of Object.keys(versions)) {
                console.log(`${version}  gt  ${version2}  =>  ${Boolean(semver.gt(version, version2))}`);
                console.log(`${version}  lt  ${version2}  =>  ${Boolean(semver.lt(version, version2))}`);
            }
        }
        // Object.keys(versions).forEach((version) => {
        //     expect().to.equal(versions[version], `Version ${version} should ${versions[version] ? 'be' : 'NOT be'} valid`);
