...
const isfileHashValid = async (providedHash, PATH) => {
  ...
  if (PATH && PATH.endsWith(".xz")) {
    ...
    if (result.success) {
      ...
      logs.warn(
        `IPFS downloaded file ${PATH} is invalid: Failed .xz verification: ${
          result.message
        }`
      );
      return false;
...
const downloadHandler = (HASH, PATH, logChunks, options = {}) =>
  new Promise((resolve, reject) => {
    ...
    const trackProgress = chunk => {
      ...
      if (options.maxSize && downloadedSize > options.maxSize) {
        reject(
          Error(
            `Downloaded size exceeds maximum allowed ${options.maxSize} bytes`
          )
        );
      }
    };
