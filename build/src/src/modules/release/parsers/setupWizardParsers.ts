...
interface SetupSchemaPropValues {
  type: string;
  title: string;
  description: string;
  default?: string;
...
type SetupSchemaProperties = {
  [propId: string]: SetupSchemaPropValues;
};
...
export function setupWizard1To2(
  ...
  function addPropertiesBlock(
    properties: SetupSchemaProperties,
    required: string[],
    ...
    for (const [propId, propValues] of Object.entries(properties)) {
      setupWizardObj[propId] = {
        ...
        id: propId,
        ...(setupWizardObj[propId] || {}),
        ...pick(propValues, ["title", "description", "pattern"])
      };
