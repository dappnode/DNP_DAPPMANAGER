...
async function getManifest({ name, ver, version }) {
  ...
  const manifestUnparsed =
    (await db.get(hash)) || (await ipfs.cat(hash, { maxSize: 100000 }));
  let manifest;
  try {
  ...
  if (!manifest.image || typeof manifest.image !== "object") {
    throw Error(
      `Invalid manifest: it does not contain the expected property 'image', manifest: ${JSON.stringify(
        manifest,
        null,
        2
      )}`
  ...
  if (!manifest.image.hash) {
    throw Error(
      `Invalid manifest: it does not contain the expected property 'image.hash', manifest: ${JSON.stringify(
        manifest,
        null,
        2
      )}`
  ...
  if (isEnsDomain(name) && (manifest || {}).name !== name) {
    throw Error(
      `Package name requested: "${name}" doesn't match the name in its manifest: "${
        manifest.name
      }"`
    );
  }
