    ...
    try {
        ...
        for (const threshold of thresholds) {
            if (diskAvailableBytes < threshold.kb) {
                ...
                logs.warn(`WARNING: DAppNode has stopped these containers after the disk space reached a ${threshold.id}: \n${dnpsToStopNames.join(', ')}`);
                eventBus.emit(eventBusTag.pushNotification, {
                    id: 'diskSpaceRanOut-stoppedPackages',
                    type: 'danger',
                    title: `Disk space is running out, stopped ${dnpsToStopNames.length} DNPs`,
                    body: `Available disk space is less than a ${threshold.id}. To prevent your DAppNode from becoming unusable some DNPs where stopped: ${dnpsToStopNames.join(', ')}. Please, free up disk space and start them again.`,
                });
                ...
                if (thresholdIsActive[threshold.id]) thresholdIsActive[threshold.id] = false;
            }
        }
    } catch (e) {
